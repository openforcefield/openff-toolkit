...
## The charges generated by the toolkit don't seem to depend on the molecule's conformation! Is this a bug?

No! This is the intended behaviour. The force field parameters of a molecule should be independent of both their chemical environment and conformation so that they can be used and compared across different contexts. When applying AM1BCC charges, the toolkit achieves a deterministic output by ignoring the input conformation and producing a new conformation for the same molecule. Charges are then computed based on this conformation.
