...
@click.option("-q", "--quiet", default=False, is_flag=True,
              help="Produce less text output.")
@click.option("-u", "--update-current", default=False, is_flag=True,
...
def install(example, target, dry_run, quiet, update_current):
    ...
    try:
        ...
        if not quiet:
            echo(f"Installing dependencies from the following files into environment at {style_path(prefix)}:")
        ...
        if not dry_run:
            echo(
                style(
                    "Exception raised; cleaning up installation in progress",
                    fg="red"
                ),
                file=sys.stderr
            )
