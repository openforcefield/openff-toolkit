...
if not os.path.isdir(datapath):
    ...
    tarfile_path = os.path.join(
        get_data_file_path("molecules"), "AlkEthOH_tripos.tar.gz"
    )
    import tarfile
...
for mol_idx, mol_filepath in enumerate(mol_filepaths):
    ...
    try:
        ...
        energies = compare_amber_smirnoff(
            prmtop_filepath, inpcrd_filepath, forcefield, molecule, ignore_charges=True
        )
