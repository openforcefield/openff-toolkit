...
class VirtualSiteHandler(_NonbondedHandler):
    ...
    class VirtualSiteType(vdWHandler.vdWType):
        ...
        def to_openff_particle(
            self, orientations: List[Tuple[int, ...]]
        ) -> _VirtualSite:

            values_dict = {"distance": self.distance}

            if self.type == "MonovalentLonePair":
                values_dict["in_plane_angle"] = self.inPlaneAngle
            if self.type in {"MonovalentLonePair", "DivalentLonePair"}:
                values_dict["out_of_plane_angle"] = self.outOfPlaneAngle

            particle_class = {
                "BondCharge": BondChargeVirtualSite,
                "MonovalentLonePair": MonovalentLonePairVirtualSite,
                "DivalentLonePair": DivalentLonePairVirtualSite,
                "TrivalentLonePair": TrivalentLonePairVirtualSite,
            }[self.type]
        ...
        def to_openmm_particle(
            self, orientation: Tuple[int, ...]
        ) -> openmm.LocalCoordinatesSite:

            openff_particle = self.to_openff_particle([orientation])
            return openff_particle.get_openmm_virtual_site(orientation)
        ...
        for parent_index, parameters in matches_by_parent.items():
            for parameter, orientations in parameters:
                for orientation in orientations:
                    ...
                    sigma = _compute_lj_sigma(
                        sigma=parameter.sigma, rmin_half=parameter.rmin_half
                    )

                    index_system = system.addParticle(0.0)
                    index_force = force.addParticle(charge, sigma, parameter.epsilon)
                    assert index_system == index_force

                    system.setVirtualSite(index_system, openmm_particle)

                    for index, change_increment in zip(
                        orientation_indices, parameter.charge_increment
                    ):

                        atom_charge, *atom_vdw = force.getParticleParameters(index)
                        atom_charge += change_increment
                        force.setParticleParameters(index, atom_charge, *atom_vdw)

                    # We can't yet add exceptions as these are generated by the
                    # top level force field...

        self._create_openff_virtual_sites(matches_by_parent)

    @staticmethod
    def _create_openff_virtual_sites(matches_by_parent):

        # keep track of which atoms we've already added by storing a tuple of
        # (parent_reference_atom_index, v_site_name, reference_molecule)
        seen_molecules = set()

        for topology_parent_index, parameters in matches_by_parent.items():

            for parameter, orientations in parameters:

                reference_molecule = orientations[0].reference_molecule
                reference_index = orientations[0].reference_atom_indices[
                    parameter.parent_index
                ]

                hash_key = (reference_index, parameter.name, reference_molecule)

                if hash_key in seen_molecules:
                    continue

                openff_particle = parameter.to_openff_particle(
                    [orientation.reference_atom_indices for orientation in orientations]
                )
                reference_molecule._add_virtual_site(openff_particle)

                seen_molecules.add(hash_key)

    def create_openff_virtual_sites(self, topology: Topology):
        """Modifies the input topology to contain VirtualSites assigned by this handler.

        Parameters
        ----------
        topology
            The topology to add virtual sites to.
        """
