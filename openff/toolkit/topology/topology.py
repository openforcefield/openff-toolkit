...
class Topology(Serializable):
    ...
    def visualize(self):
        ...
        with StringIO() as filelike:
            PDBFile.writeFile(openmm_top, openmm_positions, filelike, keepIds=False)
            pdb_str = filelike.getvalue()
        return nglview.show_text(pdb_str)

    def get_positions(self):
        """
        Return an array of the positions of the entire topology.

        Note that modifying this array will not update the positions in the topology.
        """
    ...
    def set_positions(self, array):
        """
        Set the positions in a topology from a single array.

        Parameters
        ==========

        array : Array with shape (n_atoms, 3)
            Positions for the topology. Should be a unit-wrapped array with
            dimensions of length, or else a raw NumPy array in units of
            Angstroms
        ...
        if isinstance(array, Quantity):
            array = array.m_as(unit.nanometer)
        elif isinstance(array, np.ndarray):
            array = Quantity(array, unit.angstrom).m_as(unit.nanometer)
        else:
            raise ValueError("array should be a raw NumPy array or OpenFF Quantity")

        array = np.asarray(array) * unit.nanometer
