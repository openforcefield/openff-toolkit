...
class MonovalentLonePairVirtualSite(VirtualSite):
    ...
    def local_frame_position(self):
        ...
        distance_unit = self._distance.unit
        unitless_distance = self._distance.value_in_unit(distance_unit)
        pos = unit.Quantity(
            [
                unitless_distance * np.cos(theta) * np.cos(psi),
                unitless_distance * np.sin(theta) * np.cos(psi),
                unitless_distance * np.sin(psi),
            ],
            unit=distance_unit,
        )
