...
toolkit_file_read_formats = ["SDF", "MOL", "SMI"]  # TODO: Add TDT support

class RDKitModuleUnavailable:
    def __getattr__(self, attr):
        raise ToolkitUnavailableException(
            f"The required toolkit rdkit.Chem is not available. " +
            toolkit_installation_instructions
            )
...
try:
    ...
    Chemf = RDKitModuleUnavailable()
    _is_installed = False


if _is_installed:
    from rdkit import __version__ as toolkit_version
...
def toolkit_file_write_formats():
    ...
    """
    return list(_toolkit_file_write_formats)
...
def from_object(obj, allow_undefined_stereo=False, _cls=None):
    ...
    if isinstance(obj, Chem.Mol):
        return from_rdkit(obj, allow_undefined_stereo=allow_undefined_stereo)
    raise NotImplementedError(f"Cannot create Molecule from {type(obj)} object")

@publish()
def from_pdb_and_smiles(
    file_path, smiles, allow_undefined_stereo=False, _cls=None
):
    """
...
def from_file_obj(
    ...
    if (file_format == "MOL") or (file_format == "SDF"):
        ...
        for rdmol in Chem.ForwardSDMolSupplier(file_obj): # XXX doesn't match from_file()!
            mol = from_rdkit(rdmol, _cls=_cls)
            mols.append(mol)
    ...
    if file_format == "SMI":
        ...
        lines.remove("")
        for line in lines:
            mol = from_smiles(line, _cls=_cls)
            mols.append(mol)
...
def to_file_obj(molecule, file_obj, file_format):
    ...
    file_format = file_format.upper()
    rdmol = to_rdkit(molecule)
    try:
        writer = _toolkit_file_write_formats[file_format](file_obj)
        writer.write(rdmol)
        writer.close()
...
def enumerate_stereoisomers(
    ...
    # create the molecule
    rdmol = to_rdkit(molecule=molecule)
    ...
    for isomer in isomers:
        ...
        mol = from_rdkit(isomer, _cls=molecule.__class__)
        if mol != molecule:
...
def generate_conformers(
    molecule, n_conformers=1, rms_cutoff=None, clear_existing=True, _cls=None
):
    """
...
def _elf_is_problematic_conformer(
    ...
    * contain a trans-COOH configuration. The trans conformer is discarded because
    ...
    carboxylic_acid_matches = _find_smarts_matches(
...
def _elf_prune_problematic_conformers(
    ...
    * Problematic conformers are flagged by the
    ...
    for i, conformer in enumerate(molecule.conformers):
        ...
        is_problematic, reason = _elf_is_problematic_conformer(
            molecule, conformer
        )
...
def _elf_select_diverse_conformers(
    ...
    * As the selection is greedy there is no guarantee that the selected conformers
    ...
    rms_matrix = _elf_compute_rms_matrix(molecule)
...
def apply_elf_conformer_selection(
    ...
    * Only heavy atoms are included when using the RMS to select diverse conformers.
    ...
    assign_partial_charges(molecule_copy, "mmff94")

    conformer_energies = [
        (
            _elf_compute_electrostatic_energy(molecule_copy, conformer),
            conformer,
        )
    ...
    diverse_conformers = _elf_select_diverse_conformers(
...
def to_rdkit(molecule, aromaticity_model=DEFAULT_AROMATICITY_MODEL):
    ...
    _assign_rdmol_bonds_stereo(molecule, rdmol)
...
def find_smarts_matches(molecule, smarts, aromaticity_model="OEAroModel_MDL"):
    ...
    """
    rdmol = to_rdkit(molecule, aromaticity_model=aromaticity_model)
...
def _detect_undefined_stereo(rdmol, err_msg_prefix="", raise_warning=False):
    ...
    undefined_atom_indices = _find_undefined_stereo_atoms(rdmol)
    undefined_bond_indices = _find_undefined_stereo_bonds(rdmol)
...
def _assign_rdmol_bonds_stereo(offmol, rdmol):
    ...
    for bond in offmol.bonds:
        ...
        if stereo_rdbond.GetStereo() != desired_rdk_stereo_code:
            _flip_rdbond_direction(flipped_rdbond, paired_bonds)
            Chem.AssignStereochemistry(rdmol, cleanIt=True, force=True)
