...
_rdkit_toolkit_wrapper = RDKitToolkitWrapper()

toolkit_name = "AmberTools"
...
def is_available():
    ...
    if not rdkit_wrapper.is_available():
        return False
    return True
...
def get_toolkit_version():
    global _toolkit_version
    if _toolkit_version is None:
        ...
        _toolkit_version = ambertools_version
...
def _modify_sqm_in_to_request_bond_orders(file_path):
    """
...
def _get_fractional_bond_orders_from_sqm_out(
    file_path, validate_elements=None
):
    """
...
def assign_fractional_bond_orders(
    ...
    if use_conformers is None:
        temp_mol.generate_conformers(
            n_conformers=1,
            toolkit_registry=_rdkit_toolkit_wrapper,
        )
    else:
    ...
    for conformer in [*temp_mol.conformers]:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with temporary_cd(tmpdir):
                ...
                _modify_sqm_in_to_request_bond_orders("sqm.in")
                ...
                conformer_bond_orders = (
                    _get_fractional_bond_orders_from_sqm_out(
