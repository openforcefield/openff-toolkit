...
class OpenEyeModuleUnavailable:
    def __init__(self, name):
        self.name = name
    def __getattr__(self, attr):
        raise ToolkitUnavailableException(
            f"The required toolkit openeye.{self.name} is not available. " +
            toolkit_installation_instructions
            )
...
try:
    ...
    _is_licensed = _is_licensed or oechem.OEChemIsLicensed()
except (ModuleNotFoundError, ImportError):
    oechem = OpenEyeModuleUnavailable("oechem")
    _is_installed = False
    
try:
    from openeye import oequacpac
    _is_licensed = _is_licensed or oequacpac.OEQuacPacIsLicensed()
except (ModuleNotFoundError, ImportError):
    oequacpac = OpenEyeModuleUnavailable("oequacpac")
    _is_installed = False

try:
    from openeye import oeiupac
    _is_licensed = _is_licensed or oeiupac.OEIUPACIsLicensed()
except (ModuleNotFoundError, ImportError):
    oeiupac = OpenEyeModuleUnavailable("oeiupac")
    _is_installed = False

try:
    from openeye import oeomega
    _is_licensed = _is_licensed or oeomega.OEOmegaIsLicensed()
except (ModuleNotFoundError, ImportError):
    oeomega = OpenEyeModuleUnavailable("oeomega")
    _is_installed = False

if _is_installed:
    from openeye import __version__ as toolkit_version
else:
...
def _import_openeye_tool(name):
    """Helper function to import and return openeye subpackaage by name, or return None"""
    try:
        return importlib.import_module("openeye." + tool)
    except (ImportError, ModuleNotFoundError):
        return None

def check_licenses():
    "Check license of supported OpenEye tools. Returns True if any are licensed, else False"
    return _is_licensed
...
def is_available():
    ...
    """
    return _is_installed and _is_licensed
...
def _get_format_setter(file_format):
    if file_format is None:
        ...
        oeformat = getattr(oechem, f"OEFormat_{normalized_format}", None)
        if oeformat is None:
            raise ValueError(
                "OpenEyeToolkitWrapper does not support the {file_format!r} format")
...
@publish()
def from_file(
    file_path, file_format, allow_undefined_stereo=False, _cls=None
    ):
    set_format = _get_format_setter(file_format)
    ifs = oechem.oemolistream(file_path)
    return _read_oemolistream_molecules(
...
@publish()
def from_file_obj(
    file_obj, file_format, allow_undefined_stereo=False, _cls=None
    ):
    if file_format is None:
        raise ValueError("Must specify the file_format for from_file_obj")
...
def _read_oemolistream_molecules(
    ...
    while oechem.OEReadMolecule(oemolistream, oemol):
        ...
        if is_sdf and hasattr(oemol, "GetConfs"):
            ...
            # charges into actual per-atom partial charges
            _turn_oemolbase_sd_charges_into_partial_charges(oemol)
            mol = from_openeye(
                oemol,
                allow_undefined_stereo=allow_undefined_stereo,
                _cls=_cls
            )
            # XXX what about setting partial_charges = None? -- APD
            mols.append(mol)

            # Check if this is an AMBER-produced mol2 file, which we can not
            # load because they use GAFF atom types.
            if is_mol2:
                _check_mol2_gaff_atom_type(mol, file_path)
...
def _iter_conformers_as_molecules(oemol):
    ...
    for conf in oemol.GetConfIter():
        # First, we turn "conf" into an OEMCMol (OE multiconformer mol), since
        # OTHER file formats really are multiconformer, and we will eventually
        # feed this into the `from_openeye` function, which is made to ingest
        # multiconformer mols.

        this_conf_oemcmol = conf.GetMCMol()

        # Then, we take any SD data pairs that were on the oemol, and copy them
        # on to "this_conf_oemcmol".  These SD pairs will be populated
        # if we're dealing with a single-conformer SDF.
        ...
        # On the other hand, these SD pairs will be populated if we're dealing
        # with a MULTI-conformer SDF.
        for dp in oechem.OEGetSDDataPairs(conf):
            oechem.OESetSDData(
                this_conf_oemcmol, dp.GetTag(), dp.GetValue()
            )
            
        # This function fishes out the special SD data tag we use for partial
        # charge ("atom.dprop.PartialCharge"), and applies those as
        # OETK-supported partial charges on the OEAtoms
        
        has_charges = _turn_oemolbase_sd_charges_into_partial_charges(
            this_conf_oemcmol
        )

        # Finally, we feed the molecule into `from_openeye`, where it converted
        # into an OFFMol
        mol = from_openeye(
            this_conf_oemcmol,
            allow_undefined_stereo=allow_undefined_stereo,
        ...
        # If the molecule didn't even have the `PartialCharges` tag, we set it
        # from zeroes to None here.
        if not has_charges:
...
def enumerate_stereoisomers(
    molecule, undefined_only=False, max_isomers=20, rationalise=True
    ):
    """
...
@publish()
def enumerate_tautomers(molecule, max_states=20):
    """
    ...
    for tautomer in oequacpac.OEEnumerateTautomers(oemol, tautomer_options):
        ...
        if taut != molecule:
            # XXX why isn't this tautomers.append(taut)
            tautomers.append(
                from_openeye(
                    tautomer, allow_undefined_stereo=True, _cls=molecule.__class__
                )
...
def _get_oedata(oeobj, name, default):
    if oeobj.HasData(name):
        return oeobj.GetData(name)
    return default
...
def from_openeye(oemol, allow_undefined_stereo=False, _cls=None):
    ...
    for oeatom in oemol.GetAtoms():
        ...
        stereochemistry = _openeye_cip_atom_stereochemistry(oemol, oeatom)

        #fractional_bond_order = _get_data(oebond, "fractional_bond_order", None) # XXX
        
        name = _get_oedata(oeatom, "name", "")
    ...
    for oebond in oemol.GetBonds():
        stereochemistry = _openeye_cip_bond_stereochemistry(oemol, oebond)

        fractional_bond_order = _get_oedata(oebond, "fractional_bond_order", None)

        molecule._add_bond(
            atom1 = map_atoms[oebond.GetBgnIdx()],
            atom2 = map_atoms[oebond.GetEndIdx()],
    ...
    # From MOL2: maybe
    # From other: maybe
    _add_any_conformers(molecule, oemol, map_atoms)
...
def _get_unspecified_stereo_message(oemol, problematic_atoms, problematic_bonds):
    ...
    lines = [
        f"OEMol has unspecified stereochemistry. oemol.GetTitle(): {oemol.GetTitle()}",
        ]
    ...
    if problematic_atoms:
        ...
        for problematic_atom in problematic_atoms:
            ...
            for oebond in oeatom.GetBonds():
                lines.append(
                    f"bond {oebond_to_str(oebond)} to atom {oeatom_to_str(oebond.GetNbr(oeatom))}"
                    )
            
    if problematic_bonds:
        lines.append(f"Problematic bonds are: {problematic_bonds}")

    return "\n".join(lines)
    
def _add_any_conformers(molecule, oemol, map_atoms):
    if not hasattr(oemol, "GetConfs"):
        return
...
def to_openeye(molecule, aromaticity_model=DEFAULT_AROMATICITY_MODEL):
    ...
    """
    if aromaticity_model.startswith("OEAroModel_") and hasattr(oechem, aromaticity_model):
    ...
    # Clean Up phase
    # The only feature of a molecule that wasn't perceived above seemed to be
    # ring connectivity, better to run it here then for someone to inquire
    # about ring sizes and get 0 when it shouldn't be
...
@publish()
def to_smiles(molecule, isomeric=True, explicit_hydrogens=True, mapped=False):
    """
...
@publish()
def canonical_order_atoms(molecule):
    """
...
@publish()
def from_inchi(inchi, allow_undefined_stereo=False, _cls=None):
    """
...
@publish()
def from_iupac(iupac_name, allow_undefined_stereo=False, _cls=None, **kwargs):
    """
...
def apply_elf_conformer_selection(
    ...
    * The selected conformers will be retained in the `molecule.conformers` list
    ...
    oe_molecule = to_openeye(molecule)
...
def assign_partial_charges(
    ...
    SUPPORTED_CHARGE_METHODS = _get_supported_charge_methods()
    ...
    if use_conformers is None:
        ...
        BuiltInToolkitWrapper._check_n_conformers( # XXX Move to builtin_wrapper?
            mol_copy,
...
def compute_partial_charges_am1bcc(
    molecule, use_conformers=None, strict_n_conformers=False
):
    """
...
def assign_fractional_bond_orders(
    molecule, bond_order_model=None, use_conformers=None, _cls=None
):
    """
...
@publish()
def get_tagged_smarts_connectivity(smarts):
    """
