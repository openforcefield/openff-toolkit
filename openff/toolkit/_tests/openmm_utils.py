...
def quantities_allclose(quantity1, quantity2, **kwargs):
    ...
    **kwargs
    ...
    """
    if isinstance(quantity1, openmm.unit.Quantity):
        ...
        quantity1 = quantity1.value_in_unit_system(openmm.unit.md_unit_system)
        quantity2 = quantity2.value_in_unit_system(openmm.unit.md_unit_system)
...
def compare_context_energies(
    ...
    if isinstance(potential_energy1, openmm.unit.Quantity):
        raise_assert(
...
def openmm_evaluate_vsites_and_energy(omm_top, omm_sys, atom_xyz_in_nm, minimize=False):
    ...
    integ = openmm.VerletIntegrator(1.0 * openmm.unit.femtoseconds)
