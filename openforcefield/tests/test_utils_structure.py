...
class TestUtilsStructure:
    ...
    def test_read_molecules(self):
        molecules = structure.read_molecules(
            "zinc-subset-tripos.mol2.gz", verbose=False
        )
...
def test_merge_system():
    ...
    structure.merge_system(
        topology0, topology1, system0, system1, positions0, positions1, verbose=True
    )
...
def test_component_combination():
    ...
    sdf_file_paths = [
        utils.get_data_file_path(os.path.join("systems", "monomers", name + ".sdf"))
    ...
    structure = parmed.openmm.topsystem.load_topology(
        topology.to_openmm(), system, pdbfile.positions
    )
    ...
    # Create System
    newsys = new_structure.createSystem(
        nonbondedMethod=openmm.app.NoCutoff, constraints=None, implicitSolvent=None
    )
    ...
    for resnr in range(len(structure.residues)):
        ...
        if sum(abscharges) == 0:
            raise Exception(
                f"Error: Residue {resnr} in cyclohexane-ethanol test system has a charge of zero, which is incorrect."
            )
