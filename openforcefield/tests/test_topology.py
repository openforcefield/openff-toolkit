...
@pytest.fixture()
def propane_w_vsites():
    """A propane with virtual sites"""
...
@pytest.fixture()
def tip5p_water():
    """A 5-point water like TIP5P water"""
...
class TestTopology:
    ...
    def test_from_molecules(self, ethane):
        """Test creation of a openforcefield Topology object from a Molecule objects"""
    ...
    def test_from_smiles_unique_mols(self, ethane, propane):
        """Test the addition of two different molecules to a topology"""
        topology = Topology.from_molecules([ethane, propane])

        assert topology.n_topology_molecules == 2
        assert topology.n_reference_molecules == 2

    def test_n_topology_atoms(self, ethane):
    ...
    def test_n_topology_atoms_with_vsites(self, ethane, tip5p_water):
        ...
        assert topology.n_topology_atoms == 0
        assert topology.n_topology_bonds == 0
        topology.add_molecule(tip5p_water)
        assert topology.n_topology_atoms == 3
        assert topology.n_topology_bonds == 2

    def test_n_topology_virtual_sites(self, ethane_w_vsites, tip5p_water):
        """Test n_atoms function"""
        topology = Topology()
        assert topology.n_topology_virtual_sites == 0
        topology.add_molecule(ethane_w_vsites)
    ...
    def test_get_bond(self, ethane, ethene):
        """Test Topology.bond function (bond lookup from index)"""
        topology = Topology()
        topology.add_molecule(ethane)
        topology.add_molecule(ethene)
        with pytest.raises(Exception):
            topology.bond(-1)
    ...
    def test_get_virtual_site(self, ethane_w_vsites, propane_w_vsites):
        """Test Topology.virtual_site function (get virtual site from index)"""
        topology = Topology()
        topology.add_molecule(ethane_w_vsites)
        assert topology.n_topology_virtual_sites == 2
        topology.add_molecule(propane_w_vsites)
    ...
    def test_topology_particles_virtualsites_indexed_last(
        self, ethane_w_vsites, propane_w_vsites
    ):
        """
    ...
    def test_topology_virtual_site_particle_start_index(self, propane_w_vsites):
    ...
    def test_topology_virtual_site_n_particles(self, propane_w_vsites, tip5p_water):
        """
    ...
    def test_topology_virtualsites_atom_indexing(self, ethanol, reversed_ethanol):
        """
        ...
        topology.add_molecule(ethanol)
        topology.add_molecule(ethanol)
        topology.add_molecule(reversed_ethanol)
    ...
    def test_is_bonded(self, propane_w_vsites):
        """Test Topology.virtual_site function (get virtual site from index)"""
        topology = Topology()
        topology.add_molecule(propane_w_vsites)
    ...
    def test_angles(self, ethane, propane):
    ...
    def test_propers(self, ethane, propane):
    ...
    def test_impropers(self, ethane, propane):
    ...
    def test_from_openmm(self, ethanol, cyclohexane):
    ...
    def test_from_openmm_missing_reference(self, ethanol):
    ...
    @requires_rdkit
    def test_to_file_multi_molecule_different_order(self, ethanol, reversed_ethanol):
        """
