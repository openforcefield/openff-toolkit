...
from openforcefield.properties import all_properties
from openforcefield.properties.estimator.components import protocols, groups


# =============================================================================================
# Property Calculation Template
# =============================================================================================
...
class CalculationTemplate:
    ...
    def __init__(self, property_type, template_id=None):

        self.property_type = property_type
        self.id = template_id or '{}Template'.format(str(self.property_type))
    ...
    @staticmethod
    def _parse_protocols_node(existing_templates, calculation_template, xml_node):
        """
    ...
    @staticmethod
    def _parse_groups_node(calculation_template, xml_node):
    ...
    @staticmethod
    def _parse_output_reference(output_type, template, xml_node):
        ...
        if protocol_id not in template.protocols:
            raise Exception('The {} calculation does not contain '
                            'a protocol with an id {}'.format(template.id, protocol_id))

        if property_name not in template.protocols[protocol_id].provided_outputs:
    ...
    def build(self):
        """Builds the template dependency graph and applies any
        groupings.
