...
* Formaldehyde, ``H2C=O``, with ``MonovalentLonePair`` virtual site(s) on the oxygen, with the aim of modeling both lone pairs. This one is subtle, since ``[#1:3]-[#6X3:2]=[:8X2:1]`` matches two unique groups of atoms (``1-3-4`` and ``2-3-4``), meaning that ``match="all_permutations`` behaves exactly the same as ``match="once"``. However, due to how these atoms are ordered in space, each particle will, in fact, not overlap. This means that one parameter does cover both lone pairs even though ``match="once"`` and the matched atoms are different. A standing issue here is that the default exclusion policy will allow these two virtual sites to interact since they have different indexed atoms (parents), causing the energy to be different than the non-virtual site parameterization. In the future, the ``exclusion_policy="local"`` will account for this, and make virtual sites that share at least one atom not interact with each other. As a special note: applying a MonovalentVirtualSite to a completely symmetric molecule, e.g. water, ``all_permutations`` can come into play, but this will apply two particles *not* on the same hydrogen, since the tagged atom with index 1 would match each hydrogen separately.
