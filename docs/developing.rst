   ...
   ToolkitRegistry
       ...
       ``ToolkitRegistry.from_object``  / ``ToolkitRegistry.from_smiles`` / ``OpenEyeToolkitWrapper.from_openeye`` / ``RDKitToolkitWrapper.from_rdkit``
        - These methods are a bit strange because they are effectively classmethods for ``FrozenMolecule`` and ``Molecule`` subclasses.
          ...
          However, a method like ``ToolkitRegistry.from_smiles`` doesn't have specific ties to any ``Molecule``-derived class.
          So as of  `PR #583 <https://github.com/openforcefield/openforcefield/pull/583>`_, ToolkitRegistry methods that produce a Molecule take a private parameter, ``_cls``, indicating the type of object to return.
