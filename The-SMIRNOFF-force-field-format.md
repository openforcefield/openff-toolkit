...
- `BondCharge`: This supports placement of a virtual site along a vector between two specified atoms, e.g. to allow for a sigma hole for halogens or similar contexts. ![BondCharge virtual site](figures/vsite_bondcharge.jpg "BondCharge virtual site"){:height="50%"}
- `MonovalentLonePair`: This is originally intended for situations like a carbonyl, and allows placement of a virtual site at a specified distance, `inPlaneAngle`, and `outOfPlaneAngle` relative to a central atom and two connected atoms. ![MonovalentLonePair virtual site](figures/vsite_monovalent.jpg "Monovalent lone pair virtual site")
- `DivalentLonePair`: This is suitable for cases like four-point and five-point water models as well as pyrimidine; a charge site lies a specified distance from the central atom among three atoms along the bisector of the angle between the atoms (if `outOfPlaneAngle` is zero) or out of the plane by the specified angle (if `outOfPlaneAngle` is nonzero) with its projection along the bisector. ![DivalentLonePair virtual site](figures/vsite_divalent.jpg "Divalent lone pair virtual site")
- `TrivalentLonePair`: This is suitable for planar or tetrahedral nitrogen lone pairs; charge sites lie above and below the central atom (e.g. nitrogen) along the vector perpendicular to the plane of the three connected atoms (2,3,4). ![TrivalentLonePair virtual site](figures/vsite_trivalent.jpg "Trivalent lone pair virtual site")
